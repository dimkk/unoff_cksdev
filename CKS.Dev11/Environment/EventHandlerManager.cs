...
using CKS.Dev11.VisualStudio.SharePoint.Deployment;
    ...
    internal class EventHandlerManager
    {
        ...
        private void RecycleSelectedPools(bool isSolutionContext)
        {
            try
            {
                ...
                {
                    string name = utils.GetApplicationPoolName(CKSDEVPackage.SharePointProjectService, project.SiteUrl.ToString());
        ...
        private void RecycleAllAppPools_Click(object sender, EventArgs arguments)
        {
            try
            {
                ...
                ProcessUtilities utils = new ProcessUtilities(this.DTE);
                string[] names = utils.GetAllApplicationPoolNames(this.CKSDEVPackage.SharePointProjectService);
            ...
            if (Action == vsBuildAction.vsBuildActionBuild || Action == vsBuildAction.vsBuildActionRebuildAll)
            {
                ...
                if (spProjects.Count() > 0)
                {
                    ...
                    foreach (ISharePointProject spProject in spProjects)
                    {
                        ...
                        string appPool = new ProcessUtilities(this.DTE).GetApplicationPoolName(CKSDEVPackage.SharePointProjectService, spProject.SiteUrl.ToString());
        ...
        public void DocEvents_DocumentSaved(Document document)
        {
            ...
            if (spProject != null)
            {
                ...
                if (isAutoCopyToRoot)
                {
                    IEnumerable<QuickCopyableSharePointArtefact> items = DeploymentUtilities.ResolveProjectItemToArtefacts(CKSDEVPackage.SharePointProjectService, document.ProjectItem);
