...
using Scope = CKS.Dev11.VisualStudio.SharePoint.Commands.Info.FeatureScope;
...
namespace CKS.Dev11.VisualStudio.SharePoint.Explorer
{
    ...
    public class FeatureNodeExtension : IExplorerNodeTypeExtension
    {
        ...
        void NodeType_NodeChildrenRequested(object sender, ExplorerNodeEventArgs e)
        {
            ...
            {
                e.Node.ChildNodes.AddFolder(Resources.FeatureNodeExtension_DependenciesFolderNodeName,
                Resources.FolderNode.ToBitmap(),
            ...
            {
                e.Node.ChildNodes.AddFolder(Resources.FeatureNodeExtension_ElementsFolderNodeName,
                Resources.FolderNode.ToBitmap(),
        ...
        {
            IMenuItem copyIdItem = items.Add(Resources.FeatureNodeExtension_CopyIdNodeName, 1);
        ...
        void AddToggleFeatureMenuItem(IExplorerNode parent, IMenuItemCollection items, FeatureInfo featureDetails)
        {
            ...
            if (isEnabled == false)
            {
                IMenuItem item = items.Add(
                    String.Format(Resources.FeatureNodeExtension_ActivateNodeName, featureDetails.Scope));
                item.Click +=
                    delegate
                    {
                        try
            ...
            {
                IMenuItem item = items.Add(
                    String.Format(Resources.FeatureNodeExtension_DeactivateNodeName, featureDetails.Scope));
                item.Click +=
                    delegate
                    {
                        try
