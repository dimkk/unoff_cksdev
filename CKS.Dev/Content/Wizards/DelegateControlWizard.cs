...
{
    class DelegateControlWizard : BaseWizard
    {
        #region Fields
        ...
        public override IWizardFormExtension CreateWizardForm(DTE designTimeEnvironment, WizardRunKind runKind)
        {
            ...
            ArtifactWizardForm wiz = new ArtifactWizardForm(designTimeEnvironment, Resources.DelegateControl_WizardTitle);

            if (runKind == WizardRunKind.AsNewProject)
            {
                _delegateControlProperties = new DelegateControlProperties(Guid.NewGuid(), _deploymentProperties);
                ...
                DelegateControlPresentationModel model2 = new DelegateControlPresentationModel(_delegateControlProperties, false, designTimeEnvironment);
                DeploymentPage page = new DeploymentPage(wiz, model);
                DelegateControlPage page2 = new DelegateControlPage(wiz, model2);
                DelegateControlPropertiesPage page3 = new DelegateControlPropertiesPage(wiz, model2);
                wiz.AddPage(page);
                wiz.AddPage(page2);
                wiz.AddPage(page3);
                return wiz;
            }
            //ExtendedSharePointServices sharePointServices = BaseWizard.GetSharePointServices(
            //    WizardHelpers.GetActiveProject(designTimeEnvironment));
            ...
            _delegateControlProperties = new DelegateControlProperties(Guid.NewGuid(), _deploymentProperties);
            DelegateControlPresentationModel model3 = new DelegateControlPresentationModel(_delegateControlProperties, false, designTimeEnvironment);
            DelegateControlPage page4 = new DelegateControlPage(wiz, model3);
            DelegateControlPropertiesPage page5 = new DelegateControlPropertiesPage(wiz, model3);
            wiz.AddPage(page4);
            wiz.AddPage(page5);
            return wiz;
        ...
        public override void PopulateReplacementDictionary(Dictionary<string, string> replacementsDictionary)
        {
            ...
            replacementsDictionary["$DelegateControl$"] = _delegateControlProperties.ToString();
